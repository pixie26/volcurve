...
## 目前进展
Phase 4: 后端 API(instruments/vol.compare/csv/health
comments:
1 之后web界面的时候，尽可能告诉用户有哪些indicator 比如iv rv spot skew etc etc , 或者 strike range 或者 tenor range 的选择，以及规则 比如 1d 2w 3m 2y... 或者exact date 等等， 否则客户很难使用
2 如果后台出错，希望前台网页有个位置可以看到，后台做了啥，错误原因，suggested action等等
3 iv 需要写清 3m或者具体日子 100%或者绝对strike 之类
rv 需要写清 多少时间 比如20 trading day etc
结论明确:BNP spot 是原始价(未复权)。-- 请注明 让之后前端user知道这个事儿
