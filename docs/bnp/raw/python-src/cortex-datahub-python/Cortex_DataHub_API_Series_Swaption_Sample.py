...
def apis_call():
    # get all instruments for swaption series 
    apiInstrumentsSwaption()
    # swaption serie in Json
    apiSeries()  
    # swaption serie in Csv
    apiSeriesCsv()
    
    
def apiInstrumentsSwaption():
    try: 
        response = apiGet(url_endpoints, '/v1/instruments?type=swaption', acceptApplicationJson(headers), "instrumentsSwaption.json", http_proxy)
...
def apiSeries():
    try: 
        ...
        body = {
            "code": "SWAPTION.MXN.12M.1Y.FORWARD.OFFICIAL",
        ...
        response = apiPost(url_endpoints, 
                           ...
                           json.dumps(body),
                           "series_swaption" + body["code"] + "_" + body["startDate"] + "_" + body["endDate"] + ".json",
                           http_proxy)
    except Exception as e:
        print("An error occurred during an endpoint call (authentication ok), please retry or contact or support team")
        print(e)
...
def apiSeriesCsv():
    try: 
        ...
        response = apiPost(url_endpoints, 
                           ...
                           json.dumps(body),
                           "series_swaption" + body["code"] + "_" + body["startDate"] + "_" + body["endDate"] + ".csv",
                           http_proxy)
    except Exception as e:
        print("An error occurred during an endpoint call (authentication ok), please retry or contact or support team")
        print(e)
