...
DISCLOSURES = (
    ...
    Disclosure(
        ...
        category="assumption",
        severity="warning",
        summary="Spot 来自 BNP Cortex 原始未复权价格，RV 是 price-return RV。",
        details=[
    ...
    Disclosure(
        ...
        category="limit",
        severity="info",
        summary="BNP Cortex OpenAPI 1.60.0 对 moneyness、delta 和 sliding maturity 使用全局固定枚举。",
        details=[
            "枚举不按标的或 tenor 改变，但合法坐标不代表所选标的、日期和期限一定返回有效数据。",
            "前端必须从 capabilities 读取合法值；系统不做取整或最近档位替代。",
            "Delta 定义来自 BNP API，本系统不自行推断 delta convention。",
        ],
    ...
    Disclosure(
        ...
        category="assumption",
        severity="info",
        summary="在 BNP timeZone 实际格式完成 live 验证前，source time 与 timezone 分开保留。",
